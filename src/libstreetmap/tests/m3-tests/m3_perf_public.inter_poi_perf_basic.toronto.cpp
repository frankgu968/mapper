...
SUITE(inter_inter_path_perf_basic_public) {
    TEST_FIXTURE(MapFixture, find_path_to_point_of_interest_perf_basic) {
        ...
        path = find_path_to_point_of_interest(16646, "Reds");
        path_cost = compute_path_travel_time(path);
        CHECK(path_is_legal(16646, 2270, path));
        CHECK(path_cost <= 15.89012757597712);
        ...
        path = find_path_to_point_of_interest(62274, "Main Drug Mart");
        path_cost = compute_path_travel_time(path);
        CHECK(path_is_legal(62274, 106629, path));
        CHECK(path_cost <= 13.53090066477013);
        ...
        path = find_path_to_point_of_interest(85306, "Fox and Fiddle");
        path_cost = compute_path_travel_time(path);
        CHECK(path_is_legal(85306, 96919, path));
        CHECK(path_cost <= 9.440012676052245);
