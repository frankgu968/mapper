...
SUITE(street_queries_perf) {
	...
	// test speed of getting data from a MapData instance many times
	TEST_FIXTURE(MapFixture, map_data_instance) {
		Street street;
		MapData *mapData;
		Street* streets;
		ECE297_TIME_CONSTRAINT(200);
		mapData = MapData::instance();
		streets = mapData->getStreets();
    ...
    TEST_FIXTURE(MapFixture, street_ids_from_name_perf) {
    	...
    	// test 2000 random streetIDs
    	for (unsigned i=0; i<2000; i++) {
    		ids = find_street_ids_from_name(getStreetName(i*100 % getNumberOfStreets()));
    		pass = false;
    		for (unsigned j=0; j<ids.size(); j++) {
    			if (i*100 % getNumberOfStreets() == ids[j]) {
    				pass = true;
    			}
    		}
