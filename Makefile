...
#User library include directory
...
LIB_STREETSDATABASE_INCLUDE_DIR = include/StreetsDatabase

#What directory contains the source files for the street map static library?
LIB_STREETMAP_SRC_DIR = ./src/libstreetmap/src

#What directory contains the source files for the street map library tests?
LIB_STREETMAP_TEST_DIR = src/libstreetmap/tests

LIB_ANN_SRC_DIR = src/libstreetmap/src/ANN

#What directory contains the graphics rendition sources?
...
#
GRAPHICS_LIBS = -lX11 -lXft -lfontconfig -lGL -lGLU -lSDL -lVLGraphics-d -lVLSDL-d -lVLVG-d -lVLCore-d 
...
INCLUDE_FLAGS = -I$(LIB_STREETSDATABASE_INCLUDE_DIR) -I$(INCLUDE_DIR) -I$(SDL_DIR) -I$(SRC_GRAPHICS_DIR) -I$(LIB_STREETMAP_SRC_DIR) -I$(EXE_SRC_DIR) $(GRAPHICS_FLAGS) -I/opt/X11/include
...
LFLAGS = -L./lib -L/opt/x11/lib -L/usr/local/lib/boost -L/usr/lib/x86_64-linux-gnu
LFLAGS_LIB_STREETSDATABASE = -lstreetsdatabase -lgcov
LFLAGS_LIB_STREETMAP = -lstreetmap
OTHER_LIBS = -lreadline -pthread $(BOOST_SERIALIZATION_LIB) -lFreeType-d -lZLib-d 
...
EXE_OBJ=$(patsubst %.cpp, $(BUILD_DIR)/$(CONF)/%.o,$(wildcard $(EXE_SRC_DIR)/*.cpp $(SRC_GRAPHICS_DIR)/*.cpp))

#Objects associated with the street map library
LIB_STREETMAP_OBJ=$(patsubst %.cpp, $(BUILD_DIR)/$(CONF)/%.o,$(wildcard $(LIB_STREETMAP_SRC_DIR)/*.cpp $(LIB_ANN_SRC_DIR)/*.cpp))
